    ...
    <div className="flex items-center justify-between">
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="flex h-7 w-7 items-center justify-center rounded-md border border-border text-sm text-foreground transition-colors hover:bg-secondary hover:text-primary"
...
export function IntakeForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="glow-card rounded-xl border border-border bg-card/60 p-6 backdrop-blur-sm">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h2 className="text-base font-semibold text-foreground">Project DNA</h2>
          <p className="mt-0.5 text-xs text-muted-foreground">Define your project parameters</p>
        </div>
        <button
          type="button"
          onClick={loadDemo}
          className="inline-flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-primary transition-colors hover:bg-secondary"
        >
      ...
      <div className="flex flex-col gap-5">
        {/* Project name */}
        <div>
          <Label htmlFor="project_name" className="text-xs font-medium text-muted-foreground">Project Name</Label>
          <Input
            id="project_name"
        ...
        <div>
          ...
          <Textarea
            ...
            rows={2}
            className="mt-1.5 resize-none bg-secondary/50"
          />
        </div>

        {/* Scope size */}
        <div>
          <Label className="text-xs font-medium text-muted-foreground">Scope Size</Label>
          <div className="mt-1.5 flex rounded-lg border border-border bg-secondary/30 p-0.5">
        ...
        <div>
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-medium text-foreground">{COMPLEXITY_LABELS[formData.complexity - 1]}</span>
          </div>
          <Slider
        ...
        <div>
          ...
          <Select value={formData.tech_stack} onValueChange={(v) => update("tech_stack", v)}>
            <SelectTrigger className="mt-1.5 w-full bg-secondary/50">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        {/* Deadline */}
        <div>
          <Label htmlFor="deadline" className="text-xs font-medium text-muted-foreground">Deadline (weeks)</Label>
          <Input
            id="deadline"
            type="number"
            min={1}
            ...
            className="mt-1.5 bg-secondary/50"
          />
          {errors.deadline_weeks && <p className="mt-1 text-xs text-destructive">{errors.deadline_weeks}</p>}
        </div>

        {/* Team composition */}
        <div>
          <Label className="text-xs font-medium text-muted-foreground">Team Composition</Label>
          <div className="mt-2 flex flex-col gap-2.5 rounded-lg border border-border bg-secondary/20 p-3">
        ...
        <div>
          <div className="flex items-center justify-between">
            <Label className="text-xs font-medium text-muted-foreground">Scope Volatility</Label>
            <span className="text-xs font-medium tabular-nums text-foreground">{formData.scope_volatility}%</span>
          </div>
          <Slider
            ...
            className="mt-2"
          />
          <p className="mt-1 text-[10px] text-muted-foreground">Higher = more likely requirements change</p>
        </div>
        ...
        <Collapsible open={showAdvanced} onOpenChange={setShowAdvanced}>
          ...
          <CollapsibleContent className="mt-3">
            <div>
              <Label htmlFor="num_sims" className="text-xs font-medium text-muted-foreground">Simulations</Label>
              <Input
                id="num_sims"
                type="number"
                min={100}
        ...
        <button
          ...
          className="mt-1 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground transition-all hover:brightness-110 active:scale-[0.98] disabled:opacity-60 disabled:active:scale-100"
        >
          {isSimulating ? (
            <>
